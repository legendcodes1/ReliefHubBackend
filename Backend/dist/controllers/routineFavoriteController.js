...
import { getBatchRoutineFavoriteStatus, getFavoriteRoutines, removeRoutineFavorite, RoutineFavoriteServiceError, setRoutineFavorite, } from "../service/routineFavoriteService.js";
...
export const getFavoriteRoutinesController = async (req, res) => {
    try {
...
export const createRoutineFavoriteController = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
...
export const deleteRoutineFavoriteController = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
...
export const getRoutineFavoriteStatusController = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
